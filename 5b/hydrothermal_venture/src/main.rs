...
fn add_line_to_map(map: &mut Map, p1: &Point, p2: &Point){
    if p1.x == p2.x{
        ...
        let mut slope_x : i32 = 1;
        if (p2.x as i32 - p1.x as i32) < 0{
            slope_x = -1;
        }
        //println!("slope_x {}", slope_x);

        let mut slope_y : i32 = 1;
        if (p2.y as i32 - p1.y as i32) < 0{
            slope_y = -1;
        }
        //println!("slope_y {}", slope_y);

        let x_diff : u32  = (p2.x as i32 - p1.x as i32).abs() as u32;
        //println!("x_diff {}", x_diff);
        for i in 0..x_diff + 1{
            //println!("x in map {}", (p1.x as i32 + (i as i32 * slope_x )) as usize);
            //println!("y in map {}", (p1.y as i32 + (i as i32 * slope_y )) as usize);
            map[(p1.x as i32 + (i as i32 * slope_x )) as usize][(p1.y as i32 + (i as i32 * slope_y )) as usize] += 1;
        }
    }
}
